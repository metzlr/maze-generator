...
/* Maze constructor */
Maze::Maze(int num_cellsX_, int num_cellsY_, int num_cellsZ_, double surface_cutoff_, int detail_, double path_radius, int edge_width_, int centerX, int centerY, unsigned long seed_, bool blocky_, bool find_solution) {
    seed = seed_;
    srand(seed);
    ...
    int z_boundary = 0;
    if (!flat) {
        z_boundary = boundary_size;
    }
    
    // Generate maze entrances
    std::pair<Voxel*,Voxel*> entrance_pair;
    if (centerX <= 0 || centerY <= 0) {
        entrance_pair = generateEntrances1(z_boundary);
    } else {
        entrance_pair = generateEntrances2(z_boundary, centerX, centerY);
    }
    
    
    start = entrance_pair.first;   
    end = entrance_pair.second;

    start->setValue(1.0); 
    active_vertices.push(start);    // Add start to active vertices
    ...
    // Edit finished maze
    mesh->blend(path_radius, z_boundary, 0.5);   // Blend paths
    mesh->cutoff(flat);  // Empty outermost voxels on all sides
    // Create triangles
    if (flat) {
        mesh->triangulate2D(surface_cutoff, blocky_);  
    } else {
        mesh->depthFill(z_boundary);     // Extend the maze vertically so its 3D
...
// Entrances on opposing sides
std::pair<Voxel*, Voxel*> Maze::generateEntrances1(int z) {
    std::pair<Voxel*, Voxel*> entrance_pair(nullptr, nullptr);
    for (int i = 0; i < boundary_size; i++) {   // Create start opening
        VoxelIndex index = VoxelIndex(boundary_size, i, z);
        mesh->getVoxel(index)->setValue(1.0);
    }
    for (int i = 0; i < boundary_size; i++) {   // Create end opening
        VoxelIndex index = VoxelIndex(mesh->getNumVoxelsX()-1 - boundary_size, mesh->getNumVoxelsY()-1 - i, z);
        mesh->getVoxel(index)->setValue(1.0);
    }
    // Get start and end vertex
    Voxel* start_voxel = mesh->getVoxel(VoxelIndex(boundary_size, boundary_size, z));
    Voxel* end_voxel = mesh->getVoxel(VoxelIndex(mesh->getNumVoxelsX()-1 - boundary_size, mesh->getNumVoxelsY()-1 - boundary_size, z));
    entrance_pair.first = start_voxel;
    entrance_pair.second = end_voxel;
    return entrance_pair;
}

// One entrance on edge, other in center
std::pair<Voxel*, Voxel*> Maze::generateEntrances2(int z, int center_size_x, int center_size_y) {
    if (num_cellsX % 2 != center_size_x % 2) { center_size_x++; }
    if (num_cellsY % 2 != center_size_y % 2) { center_size_y++; }
    if (center_size_x > num_cellsX-2) center_size_x = num_cellsX - 2;
    else if (center_size_x < 2) center_size_x = 2 + num_cellsX % 2;
    if (center_size_y > num_cellsY-2) center_size_y = num_cellsY - 2;
    else if (center_size_y < 2) center_size_y = 2 + num_cellsY % 2;
    
    std::pair<Voxel*, Voxel*> entrance_pair(nullptr, nullptr);
    for (int i = 0; i < boundary_size; i++) {   // Create start opening on edge
        VoxelIndex index = VoxelIndex(boundary_size, i, z);
        mesh->getVoxel(index)->setValue(1.0);
    }
    Voxel* start_voxel = mesh->getVoxel(VoxelIndex(boundary_size, boundary_size, z));
    // Make sure it will be centered. (Even center dimension if maze dimension is even, odd if odd)
    //int center_size_x_actual = (center_size_x % 2 != 0) && (num_cellsX % 2 == 0) ? center_size_x*2 : center_size_x;
    //int center_size_y_actual = num_cellsY % 2 == 0 ? center_size_y*2 : center_size_y;

    int center_size_voxels_x = center_size_x*scale + 1 - scale;
    int center_size_voxels_y = center_size_y*scale + 1 - scale;
    int center_start_x = mesh->getNumVoxelsX()/2 - (center_size_voxels_x)/2;
    int center_start_y = mesh->getNumVoxelsY()/2 - (center_size_voxels_y)/2;
    for (int x = center_start_x; x < center_size_voxels_x + center_start_x; x++) {
        for (int y = center_start_y; y < center_size_voxels_y + center_start_y; y++) {
            mesh->getVoxel(VoxelIndex(x, y, z))->setValue(1.0);
        }
    }
    Voxel* end_voxel = mesh->getVoxel(VoxelIndex(mesh->getNumVoxelsX()/2 + center_size_voxels_x/2, mesh->getNumVoxelsY()/2 + center_size_voxels_y/2, z));
    end_voxel->setValue(-1.0);
    entrance_pair.first = start_voxel;
    entrance_pair.second = end_voxel;
    return entrance_pair;
}
...
bool Maze::generatePath(const std::vector<int>& sizes) {
    ...
    while (len > 0) {
        ...
        if (current == end) break; // Don't continue path or add to active if end vertex
